...
function init( idb ) {
    ...
    async function query( schema, store, params ) {
        ...
        const results = await new Promise( ( resolve, reject ) => {
            ...
            const objStore = await idbOpenObjStore( schema, store );
        ...
        const objStore = await idbOpenObjStore( schema, store );
        const objects = await idbReadAll( results, objStore );
        ...
        constructor( objStore, params ) {
            ...
            for( const key in params ) {
                ...
                if( index === objStore.keyPath ) {
                    iq = new PKQuery( objStore, index, test, value );
                }
                else if( objStore.indexNames.contains( index ) ) {
                    iq = new IndexQuery( objStore, index, test, value );
                }
                else {
                    iq = new ScanQuery( objStore, index, test, value );
                }
                queries.push( iq );
            }
    ...
    class PKQuery extends Query {
        ...
        openCursor( onsuccess, onerror ) {
            ...
            const request = idbOpenPK( term, objStore );
            request.onsuccess = () => {
    ...
    class IndexQuery extends Query {
         ...
         */
        openCursor( onsuccess, onerror ) {
            const { term, index, objStore } = this;
            // NOTE the abstraction leak here; the idbOpenPK/idbOpenIndex
            // functions return a request object rather than a promise;
            // returning an async iterator would probably be a better idea.
            const request = idbOpenIndex( index, term, objStore );
            request.onsuccess = () => {
                this.cursor = request.result;
                onsuccess();
            };
    ...
    class ScanQuery extends Query {
        ...
        openCursor( onsuccess, onerror ) {
            ...
            request.onsuccess = () => {
                ...
                if( cursor !== null ) {
                    const { value } = cursor;
                    // Test of the current row matches the query criteria and
