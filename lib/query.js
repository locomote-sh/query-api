...
function initQuery( idb ) {
    ...
    async function query( schema, store, params ) {
        ...
        const results = await new Promise( async ( resolve, reject ) => {
            ...
            const cursors = new QueryCursors( db, store, params );
        ...
        const objects = await idbReadAll( objStore, results );
    ...
    class QueryCursors {
        ...
        constructor( db, store, params ) {
            ...
            for( const key in params ) {
                ...
                // Open the object store so that we can read some metadata from it.
                const tx = db.transaction( store, 'readonly');
                const objStore = tx.objectStore( store );
                let iq;
                if( index === objStore.keyPath ) {
                    iq = new PKQuery( index, test, value );
                }
                else if( objStore.indexNames.contains( index ) ) {
                    iq = new IndexQuery( index, test, value );
                }
                else {
                    iq = new ScanQuery( index, test, value );
                }
                queries.push( iq );
            }
        ...
        open( onsuccess, onerror ) {
            const { _db, _store } = this;
            const tx = _db.transaction( _store, 'readonly');
            const objStore = tx.objectStore( _store );
            for( const query of this.queries ) {
                query.openCursor( objStore, onsuccess, onerror );
            }
        }
    ...
    class PKQuery extends Query {
        ...
        openCursor( objStore, onsuccess, onerror ) {
            ...
            const request = idbOpenPK( objStore, term );
            request.onsuccess = () => {
    ...
    class IndexQuery extends Query {
        ...
        constructor( objStore, index, test, value ) {
            super( objStore, index, test, value );
            this.initKeyRange();
        }
        ...
        openCursor( objStore, onsuccess, onerror ) {
            ...
            // functions return a request object rather than a promise;
            // returning an async iterator would probably be a better idea.
            const request = idbOpenIndex( objStore, index, term );
            request.onsuccess = () => {
                this.cursor = request.result;
                onsuccess();
            };
